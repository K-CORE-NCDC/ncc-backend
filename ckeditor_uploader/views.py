...
# from ckeditor_demo.demo_application.models import ExampleModel
from .utils import is_valid_image_extension, is_valid_video_extension, is_valid_file_extension
...
def get_all_files(user=None, path=""):
    """
    ...
    try:
        ...
        return
    # print('storage_list',storage_list)
    ...
    for directory in storage_list[STORAGE_DIRECTORIES]:
        ...
        yield from get_all_files(user=user, path=directory_path)


def get_files_browse_urls(user=None, typ="all"):
    """
    Recursively walks all dirs under upload dir and generates a list of
    thumbnail and full image URL's for each file found.
    """
    files = []
    for filename in get_all_files(user=user):
        ...
        if getattr(settings, "CKEDITOR_IMAGE_BACKEND", None):
            ...
            if typ == "image" and is_valid_image_extension(src):
                thumb = utils.get_media_url(utils.get_thumb_filename(filename))
                files.append(
                    {
                        "thumb": thumb,
            ...
            elif typ == "all" and (is_valid_video_extension(src) or is_valid_file_extension(src)):
                thumb = utils.get_icon_filename(filename)
                files.append(
                    {
                        "thumb": thumb,
...
def browse(request, typ):
    files = get_files_browse_urls(request.user, typ)
