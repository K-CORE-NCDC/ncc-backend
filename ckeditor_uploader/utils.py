...
fileicons_path = "{}/file-icons/".format(
    getattr(settings, "CKEDITOR_FILEICONS_PATH", "/k-corev/static/ckeditor")
)
...
FILE_EXTENSIONS = {".pdf", ".doc", ".txt", ".xls", ".file", ".ppt"}
...
def get_media_url(path):
    ...
    """
    return f"/k-corev{storage.url(path)}"
...
def is_valid_file_extension(file_path):
    extension = os.path.splitext(file_path.lower())[1]
