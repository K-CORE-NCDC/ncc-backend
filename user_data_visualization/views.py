    ...
    def post(self, request, **args):
        ...
        try:
            ...
            if first_failure:
                ...
                if success == True:
                    project_name = maincolumns["project_name"]
                    if UserDataProjects.objects.exists():
                        latest_id_project = UserDataProjects.objects.latest('id').id
                    else:
                        latest_id_project = 0

                    if UserDataExtension.objects.exists():
                        latest_id_extension = UserDataExtension.objects.latest('id').id
                    else:
                        latest_id_extension = 0

                    latest_project_id = max(latest_id_project, latest_id_extension)

                    latest_project_id = int(latest_project_id)+1
                    latest_project_id = str(latest_project_id)
                    project_name = f"{project_name}_{latest_project_id}"
                    ...
                    db_path = os.path.join(user_db_directory, project_name)
                    ...
                    file_names["available_steps"] = available_visualizations
                    try:
                        project_object = UserDataProjects.objects.get(
                            name=project_name)
                    except BaseException:
                        ...
                        project_object.save()
                    serializer = UserDataProjectsSerializer(
                        project_object, data=file_names, partial=True
                    )
                    project_id = 0
    ...
    def get(self, request, project_id=None):
        ...
        try:
            ...
            if project_id is None:
                ...
                table_data = UserDataProjects.objects.get(user=user, id=project_id)
